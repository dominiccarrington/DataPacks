...
execute at @a[gamemode=survival] run summon minecraft:armor_stand ~ ~ ~ {NoGravity:1,Invisible:1,CustomName:'"DeathSwap"'}
execute at @a[gamemode=survival] as @p run scoreboard players operation @e[type=minecraft:armor_stand,name=DeathSwap,sort=nearest,limit=1] player = @s player
scoreboard players add @e[type=minecraft:armor_stand,name=DeathSwap] player 1
